...
boolean isMessageReady(){
  ...
  // DEBUG
  Serial.print("my variables status: ");
  Serial.print(actor);
  Serial.print(" : ");
  ...
  if (actor[0]!=0&&(activity[0]!=0||project[0]!=0)){
    ...
    if (client.connected()){
      client.stop();
      //DEBUG
      Serial.println("done GETTag disconnecting");
      // end DEBUG
    }
    return true;
  }
...
void requestTagInfo(){
  ...
  //  }
  // Make a HTTP request:
  Serial.println("making request for: ");
  Serial.println(tag);
